...
bool SDCard::init()
{
    ...
    SPI_SD::init(&DDRB, &DDRB, &DDRB, &DDRB, PB0, PB1, PB2, PB3, &PORTB);
#elif __AVR_ATmega328P__
    SPI_SD::init(&DDRB, &DDRB, &DDRB, &DDRB, PB2, PB5, PB3, PB4, &PORTB);
#else
    ...
    if (type == Type::SDv2) {
        ...
        if ((SPI_SD::read() & 0XC0) == 0XC0) type = Type::SDHC;
        // discard rest of ocr - contains allowed voltage range
        for (uint8_t i = 0; i < 3; i++) SPI_SD::read();
    }
...
uint8_t SDCard::send_cmd(uint8_t cmd, uint32_t arg)
{
    ...
    for (uint8_t i = 0; ((status = SPI_SD::read()) & 0X80) && i != 0XFF; i++);
    return status;
}
...
bool SDCard::write_block(uint32_t block_no, const uint8_t* src)
{
    ...
    if(send_cmd(CMD13, 0) || SPI_SD::read()) {
